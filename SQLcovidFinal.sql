...
--DEATH PECENTAGE IN NIGERIA
SELECT location, date, total_cases,total_deaths, ((cast(total_deaths as float))/(cast(total_cases as float))) * 100 as deathPecentagr
FROM PortfolioProjects..covidDeath$
WHERE location like '%Nigeria%'
ORDER BY 2,3;

--DEATH PECENTAGE IN SOUTH AFRICA
SELECT location, date, total_cases,total_deaths, ((cast(total_deaths as float))/(cast(total_cases as float))) * 100 as deathPecentagr
FROM PortfolioProjects..covidDeath$
WHERE location like '%SOUTH AFRICA%'
ORDER BY 2,3;

--DEATH PECENTAGE IN GHANA
SELECT location, date, total_cases,total_deaths, ((cast(total_deaths as float))/(cast(total_cases as float))) * 100 as deathPecentagr
FROM PortfolioProjects..covidDeath$
...
--SHOWS WHAT POPULATION GOT COVID INGERIA
SELECT location, date, total_cases,population, ((cast(total_cases as float))/(population)) * 100 as pecentofPopulationInfected
FROM PortfolioProjects..covidDeath$
WHERE location like '%Nigeria%'
ORDER BY 2,3;

--SHOWS WHAT POPULATION GOT COVID IN GHANA
SELECT location, date, total_cases,population, ((cast(total_cases as float))/(population)) * 100 as pecentofPopulationInfected
FROM PortfolioProjects..covidDeath$
WHERE location like '%Nigeria%'
ORDER BY 2,3;

--SHOWS WHAT POPULATION GOT COVID IN SOUTH AFRICA
SELECT location, date, total_cases,population, ((cast(total_cases as float))/(population)) * 100 as pecentofPopulationInfected
FROM PortfolioProjects..covidDeath$
WHERE location like '%South Africa%'

--SHOWS WHAT POPULATION GOT COVID IN ZIMBABWE
SELECT location, date, total_cases,population, ((cast(total_cases as float))/(population)) * 100 as pecentofPopulationInfected
FROM PortfolioProjects..covidDeath$
WHERE location like '%ZIMBABWE%'
ORDER BY 2,3;
...
--LOOKING AT COUNTRIES WITH HIGHEST POPULATION INFECTION RATE COMPARE TO POPULATION
--SHOWS POPULATION GOT COVID IN GHANA, NIGERIA SOUTH AFRICAL AND ZIMBABWE
SELECT location, population, MAX(total_cases) as highestInfectionCount, MAX((cast(total_cases as float))/(population)) * 100 as pecentPopulationInfected
FROM PortfolioProjects..covidDeath$ 
WHERE location IN ('Nigeria', 'South Africa', 'Ghana', 'Zimbabwe' )
GROUP BY LOCATION, population
ORDER BY pecentPopulationInfected DESC;
